...
void faireFIFO_Ou_Tourniquet(_ListeTrnq *Tr, ListeP lst, char *algo)
{
	...
	ListeP Ls = {lst.nbMAX,lst.nbproc};
	L.T   = (BCP*)calloc(lst.nbMAX,sizeof(BCP));
	Ls.T  = (BCP*)calloc(lst.nbMAX,sizeof(BCP));
	...
	for(int i = 0; i<lst.nbproc; i++) Ls.T[i] = L.T[i];

	taille_agrandi_Gant();
	long *p = NULL; //On doit libérer la mémoir allouée dynamiquement sur la fonction d'ordonnancement,cette variable nous sert de corps de libération
	if(!strcmp(algo,"TOURNIQUET"))
		 p = Ord_Tourniquet(Tr,L,"TOURNIQUET");
	else p = Ord_Tourniquet(Tr,L,"FIFO");

	for(int i = 0; i<L.nbproc;i++) printf(" TR%d : %ld \n",i+1,*p++ );
	...
	afficher_Le_Schema_Temporelle(Ls);//return 0;
	//Ordonnancement
