...
void initialiser_La_Liste(ListeP *L,int NMAX)
{
	L->nbMAX  = NMAX;
	L->nbproc =    0;
	L->f = NULL; //Pointeur sur FILE (Le fichier qui prendra tous les processus lu par le programme)
...
FILE *fprintp(BCP tab[], int deb, int fin, FILE *stream)
{
	...
	if(stream == NULL)//Il n'ya pas encore un fichier pour ce programme
	{
		...
		if(opendir("_Sauvegarde") == NULL) mkdir("_Sauvegarde",0777);
...
{	printf("%s\n",init);
	...
	printf("\n Pour retourner Dans le menu Principal tapez 0\n\n");
	int n = 0;
	do
	{
...
void Afficher_En_Resume(ListeP L, char type )
{
	if(la_liste_est_vide(L))
	{
		clear();
	...
	for ( i = 0; i < L.nbproc; i++)
	{
		...
		/*******************************Somme des 2 Temps********************************/
			 if(rot >= 100)            nbchar += printf("               %d",rot );
		else if(rot>=10 && rot <100)   nbchar += printf("                %d",rot );
		else if(rot <10)               nbchar += printf("                 %d",rot );



		if(i == L.nbproc-1)
		{
			for(int j = 0;j < 100 - nbchar;j++ ) printf("_");
			printf("\e[0m|\n");
		}
		else
		{
			for(int j = 0;j < 100 - nbchar;j++ ) printf(" ");
			printf("|\n");
		}
	...
	while(1)
	{
		if(ch[0] < '0' && ch[0] > '2') printf("\n\n\t\t\t\t\t\e[\e[40;31m Ce choix n'existe pas [_INCORRECT_CHOICE_ ] \e[0;30m");
		...
		list.T = (BCP*)calloc(L.nbMAX,sizeof(BCP));
		...
		switch(ch[0])
		{
			...
			case '3' : clear(); Afficher_En_Resume(list, 'b');     ;break;
			default  : clear(); printf("\n\n\t\t\t\t\e[\e[47;31m Ce choix n'existe pas [_INCORRECT_CHOICE_] \e[30m");
						Afficher_En_Resume(L, 'b');
		}
	}
}
/**
* Affichage d'une seule processus
* Avec une structure de ligne d'un tableau
* Cette fonctiion sera appélée en dans un boucle en
* donnant différent processus
* @Affichage_Proc_One
*/
...
void __horloge_TIMER(int n)
{
	...
	for (int i = 0; i < n; ++i)
	{
		(i<10) ? printf("%s %d",cel,i)
				 :
			     printf("%s%d",cel,i);

	     if(i%2 == 0) strcpy(cel,"\e[40;37m");
	     else 		  strcpy(cel,"\e[47;30m");
	};
	printf("%s|\n",init);
}
...
void faireFIFO_Ou_Tourniquet(_ListeTrnq *Tr, ListeP lst, char *algo)
{
	...
	printf("\n\n\t\t\t\t \e[41;30m Voici la liste des processus a simule\e[0m\n");
