...
{
	/*Vu que nous allons travailler directement sur le tableau contenant les processus, 
	* nous allons nous assurer d'en garder des copies non modifés*/
	ListeP _LISTE = {liste.nbMAX,liste.nbproc};;
	_LISTE.T = calloc(_LISTE.nbproc,sizeof(BCP)); /*Le mieux à faire est de créer un tableau dynamique*/

	for (int i = 0; i < _LISTE.nbproc; ++i)	_LISTE.T[i] = liste.T[i];
	...
	long _TIMER         =  _LISTE.T[0].DA ;/* Horloge du processeur pour l'ordonnancement                    */
	...
	long *rotate = calloc(_LISTE.nbproc,sizeof(long)); 
	//Initialisation du tableau avec le temps d'exécution de chaque BCP
	for (int indice = 0; indice < _LISTE.nbproc; ++indice) rotate[indice] = -1;
	...
	{
		while(j < _LISTE.nbproc) //Boucle contenant l'ordonnancement
		{
			p = &_LISTE.T[j]; /*  Le processus courant  */
			...
			if( ( (i < p->nbEX  && p->TEX[i] <= Tr->Q) ) || (p->nbEX == 0 && m < p->nbES) )
			{
				...
				switch(etatProc(Tr,*p,_TIMER) == es || etatProc(Tr,*p,_TIMER) == bloque)
				{
					...
					case FAUX : /* Le processus n'est pas entrain de faire les ES ou n'est pas bloqué */
						...
						/*********************************************** Le cas ELU  ***********************************************/
						if(_LISTE.nbproc == 1 && etatProc(Tr,*p,p->fin_es-1) == es) //Cas particulier
						...
						/*********************************************** Le cas PRET  ***********************************************/
						for (int r = 0; r < _LISTE.nbproc; r++)
						{
							BCP processus = _LISTE.T[r];
							...
							else if(processus.eluC<processus.nbEX && (etatProc(Tr,processus,_TIMER) == -1) )
							{
								...
								if( processus.ID != _LISTE.T[0].ID && recherche(Tr,processus.ID,1,pret) == NULL )
								{
					...
					case VRAI : /* Le processus est entrain de faire les ES ou est bloqué*/
						...
						for(int r = 0; r < _LISTE.nbproc; r++)
						{
							if(aTermine(_LISTE.T[r]) == VRAI || etatProc(Tr,_LISTE.T[r],_TIMER) != es) continue;
							if(etatProc(Tr,_LISTE.T[r],_TIMER) == es)
							{
								a *= 1;
								if(prem == -1) prem = r;
							}
	...
	cond_ES :	_FIN_DERNIER_ES = effectuer_Les_ES(p, Tr, _LISTE, _FIN_DERNIER_ES, _TIMER,j,rotate);
				i = p->eluC;
				m = p->esC;
		...
		j = 0;
	}while( _FINI_EXECUTION_ < _LISTE.nbproc  );
